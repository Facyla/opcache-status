...
// Script from https://github.com/rlerdorf/opcache-status/
// Translation FR + lights changes by Facyla 2020
define('THOUSAND_SEPARATOR',true);

if (!extension_loaded('Zend OPcache')) {
	echo '<div style="background-color: #F2DEDE; color: #B94A48; padding: 1em;">Extension Zend OPcache non chargée.</div>';
}
...
class OpCacheDataModel{
	...
	public function getPageTitle(){
		return 'PHP ' . phpversion() . " avec OpCache {$this->_configuration['version']['version']}";
	}
	...
	public function getConfigDataRows(){
		...
		foreach ($this->_configuration['directives'] as $key => $value) {
			if ($value === false) { $value = 'false'; }
			if ($value === true) { $value = 'true'; }
			if ($key == 'opcache.memory_consumption') { $value = $this->_size_for_humans($value); }
			$rows[] = "<tr><th>$key</th><td>$value</td></tr>\n";
		}
		return implode("\n", $rows);
	}
	...
	private function _format_value($value){
		if (THOUSAND_SEPARATOR === true) { return number_format($value); }
		return $value;
	}
	
	private function _size_for_humans($bytes){
		if ($bytes > 1048576) { return sprintf('%.2f&nbsp;Mo', $bytes / 1048576); }
		if ($bytes > 1024) { return sprintf('%.2f&nbsp;Ko', $bytes / 1024); }
		return sprintf('%d&nbsp;octets', $bytes);
	}
	...
	<div id="container">
		...
		<div class="tabs">
			<div class="tab">
				...
				<label for="tab-status">Statut</label>
				<div class="content">
					<table>
			...
			<div class="tab">
				...
				<div class="content">
					<table style="font-size:0.8em;">
						<tr>
							<th width="10%">Hits</th>
							<th width="20%">Mémoire</th>
							<th width="70%">Chemin</th>
						</tr>
			...
			<div class="tab">
				...
				<label for="tab-visualise">Visualiser la partition</label>
				<div class="content"></div>
			</div>
		</div>
		<div id="graph">
			<form>
				<label><input type="radio" name="dataset" value="memory" checked> Mémoire</label>
				<label><input type="radio" name="dataset" value="keys"> Cache des clefs</label>
				<label><input type="radio" name="dataset" value="hits"> Hits</label>
				<label><input type="radio" name="dataset" value="restarts"> Redémarrages</label>
			</form>
			<div id="stats"></div>
		</div>
	</div>
	<div id="close-partition">&#10006; Fermer la visualisation</div>
		...
		function set_text(t) {
			if (t === "memory") {
				d3.select("#stats").html(
					"<table><tr><th style='background:#B41F1F;'>Utilisée</th><td><?php echo $dataModel->getHumanUsedMemory()?></td></tr>"+
					"<tr><th style='background:#1FB437;'>Libre</th><td><?php echo $dataModel->getHumanFreeMemory()?></td></tr>"+
					"<tr><th style='background:#ff7f0e;' rowspan=\"2\">Gaspillée</th><td><?php echo $dataModel->getHumanWastedMemory()?></td></tr>"+
					"<tr><td><?php echo $dataModel->getWastedMemoryPercentage()?>%</td></tr></table>"
				);
			} else if (t === "keys") {
				d3.select("#stats").html(
					"<table><tr><th style='background:#B41F1F;'>Clefs mises en cache</th><td>"+format_value(dataset[t][0])+"</td></tr>"+
					"<tr><th style='background:#1FB437;'>Espace libre</th><td>"+format_value(dataset[t][1])+"</td></tr></table>"
				);
			} else if (t === "hits") {
				d3.select("#stats").html(
					"<table><tr><th style='background:#B41F1F;'>Manquées</th><td>"+format_value(dataset[t][0])+"</td></tr>"+
					"<tr><th style='background:#1FB437;'>Hits</th><td>"+format_value(dataset[t][1])+"</td></tr></table>"
				);
			} else if (t === "restarts") {
				d3.select("#stats").html(
					"<table><tr><th style='background:#B41F1F;'>Mémoire</th><td>"+dataset[t][0]+"</td></tr>"+
					"<tr><th style='background:#1FB437;'>Manuel</th><td>"+dataset[t][1]+"</td></tr>"+
					"<tr><th style='background:#ff7f0e;'>Clefs</th><td>"+dataset[t][2]+"</td></tr></table>"
				);
			}
		}
		function change() {
